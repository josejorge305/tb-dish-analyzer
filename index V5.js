...
function makeTrace(endpoint, searchParams, env, extras = {}) {
  const pickStr = (k, def = undefined) => {
    const v = searchParams.get(k);
    return v == null || v === '' ? def : String(v);
  };
  const pickInt = (k, def = undefined) => {
    const v = searchParams.get(k);
    if (v == null || v === '') return def;
    const n = parseInt(v, 10);
    return Number.isFinite(n) ? n : def;
  };
  const pickFloat = (k, def = undefined) => {
    const v = searchParams.get(k);
    if (v == null || v === '') return def;
  ...
  // host hint (not a header; just helpful inside the trace object)
  const host =
    (env && (env.UBER_RAPID_HOST || env.RAPIDAPI_HOST || env.CF_PAGES_URL || env.HOSTNAME)) ||
    undefined;

  return {
    endpoint,
    // common query params we expose
    query: pickStr('query', undefined),
    address: pickStr('address', undefined),
    locale: pickStr('locale', undefined),

    // pagination + sizing
    page: pickInt('page', undefined),
    maxRows: pickInt('maxRows', undefined) ?? pickInt('top', undefined),

    // geo hints (if provided)
    lat: pickFloat('lat', undefined),
    lng: pickFloat('lng', undefined),
    radius: pickInt('radius', undefined),

    // infra hints
    host,

    // we’ll fill this later where known:
    used_path: undefined,

    // allow callers to merge extra details
    ...extras,
  };
}
...
(function ensureLLMHelpers() {
  ...
  if (typeof globalThis.normalizeLLMItems !== 'function') {
    globalThis.normalizeLLMItems = function normalizeLLMItems(rawItems = [], tag = 'llm') {
  ...
  if (typeof globalThis.dedupeItemsByTitleSection !== 'function') {
    globalThis.dedupeItemsByTitleSection = function dedupeItemsByTitleSection(items = []) {
      const seen = new Map(),
        keep = [];
  ...
  if (typeof globalThis.callGrokExtract !== 'function') {
    globalThis.callGrokExtract = async function callGrokExtract(env, query, address) {
      ...
      if (!_isValidUrl(url)) return { ok: true, items: [], note: 'grok_invalid_url' };
      if (!key) return { ok: true, items: [], note: 'grok_missing_key' };
      try {
        const res = await fetch(url, {
          method: 'POST',
          headers: { 'content-type': 'application/json', authorization: `Bearer ${key}` },
          body: JSON.stringify({ task: 'menu_extract', query, address, schema: 'menu_items_v1' }),
        });
        if (!res.ok) return { ok: false, items: [], error: `grok HTTP ${res.status}` };
        const js = await res.json().catch(() => ({}));
        const items = Array.isArray(js.items) ? js.items : [];
        return { ok: true, items };
      } catch (e) {
        return { ok: false, items: [], error: `grok fetch error: ${String(e?.message || e)}` };
      }
    };
  }

  if (typeof globalThis.callOpenAIExtract !== 'function') {
    globalThis.callOpenAIExtract = async function callOpenAIExtract(env, query, address) {
...
Return JSON with an "items" array. Each item: {title, description, section, price_text?, calories_text?}.`;
      ...
      try {
        ...
        if (!res.ok) return { ok: false, items: [], error: `openai HTTP ${res.status}` };
        const js = await res.json().catch(() => ({}));
        let payload = {};
        try {
        ...
        return { ok: false, items: [], error: `openai fetch error: ${String(e?.message || e)}` };
      }
    };
  }
})();
...
function extractMenuItemsFromUber(raw, queryText = '') {
  ...
  const results =
    (raw?.data?.results && Array.isArray(raw.data.results) && raw.data.results) ||
  ...
  if (q && results.length) {
    ...
    chosen =
      bestScore > 0 ? scored.filter((s) => s.score === bestScore).map((s) => s.r) : [scored[0].r];
  }
  ...
  function normalizePriceFields(mi) {
    ...
    const price_display =
      mi.priceTagline ||
      (Number.isFinite(price) ? `$${(price / 100).toFixed(2)}` : mi.price_display || '') ||
      '';
  ...
  function deriveCaloriesDisplay(mi, price_display) {
    ...
    const calNum =
      (mi?.nutrition && Number.isFinite(mi.nutrition.calories) && mi.nutrition.calories) ||
    ...
    const t = String(price_display || mi?.itemDescription || mi?.description || '');
  ...
  function normalizeItemFields(it) {
    ...
    if (out.calories_display != null) out.calories_display = clean(out.calories_display);
  ...
  for (const r of chosen) {
    ...
    for (const section of sections) {
      ...
      const catalogItems =
        ...
        [];
      for (const mi of catalogItems) addItem(makeItem(mi, sectionName, restaurantName));
    }

    // featured
    if (Array.isArray(r.featuredItems)) {
      for (const mi of r.featuredItems) addItem(makeItem(mi, 'Featured', restaurantName));
    }
  }
...
function filterAndRankItems(items, searchParams, env) {
  ...
  if (skipDrinks) {
    candidates = candidates.filter((it) => !isDrink(it.name || it.title || '', it.section));
  }
  if (skipParty) {
    candidates = candidates.filter((it) => !isPartyPack(it.name || it.title || ''));
  }
...
function isDrink(name = '', section = '') {
  ...
  const s = (section || '').toLowerCase();
  return (
    /\b(drink|beverage|soda|coke|sprite|fanta|tea|coffee|juice|shake|mcflurry|water)\b/.test(n) ||
...
function isPartyPack(name = '') {
  ...
  return (
    /\b(20|30|40|50)\s*(pc|piece|pieces)\b/.test(n) || /\b(family|party|bundle|pack)\b/.test(n)
  );
}
...
function parseOCRToCandidates(fullText) {
  ...
  function pushItem(title, price, section) {
    ...
    if (!name) return;
    items.push({ title: name, price_text: `$${price}`, section: section || null });
  }
...
function pickMenuImageFromHtml(html, baseUrl) {
  ...
  function score(url) {
    ...
    if (/\b(food|foods|dishes|lunch|dinner|brunch|dessert|beverage|drinks|wine|beer)\b/.test(u))
      s += 3;
    if (/\.(jpg|jpeg|png|webp)(\?|$)/.test(u)) s += 2;

    // down-rank sprites, logos, favicons, tiny thumbs, social, tracking
    if (/sprite|icon|logo|favicon|thumb|tracking|pixel|social|twitter|facebook|instagram/.test(u))
      s -= 5;
...
function rankCanon(items = [], topN = 25) {
  const scored = items.map((it, idx) => {
    ...
    if (/\bmost popular\b|\bpopular items\b|\bfeatured\b|\bbestsellers\b|\brecommended\b/.test(s))
      score += 100;
...
async function recipeCacheWrite(env, key, payload) {
  ...
  if (!kv) return false;
  try {
    await kv.put(key, JSON.stringify({ savedAt: new Date().toISOString(), ...payload }), {
...
async function handleDebugVision(request, env) {
  ...
  if (!env.GCP_VISION_API_KEY)
    return json({ ok: false, error: 'Missing GCP_VISION_API_KEY' }, { status: 500 });
...
async function callEdamam(env, dish, cuisine = '', lang = 'en') {
  ...
  if (!appId || !appKey) {
    return { ok: true, items: [], note: 'edamam_missing_keys', provider: 'edamam' };
  }
  ...
  try {
    ...
    if (!r.ok) {
      return { ok: false, items: [], error: `edamam HTTP ${r.status}`, provider: 'edamam' };
    }
...
async function callOpenAIRecipe(env, dish, cuisine = '', lang = 'en') {
  const key = (env.OPENAI_API_KEY || '').trim();
  const base = ((env.OPENAI_API_BASE || 'https://api.openai.com') + '').replace(/\/+$/, '');
  if (!key) {
    return {
      ...
      recipe: null,
      ingredients: [],
      note: 'openai_missing_key',
      provider: 'openai',
    };
  }

  const prompt = `Create a clear home-kitchen recipe for "${dish}"${
    cuisine ? ` (cuisine: ${cuisine})` : ''
  }.
Return strict JSON:
{
  "recipe": { "name": "...", "steps": ["..."], "notes": ["..."] },
  "ingredients": [ { "name": "...", "qty": <number|null>, "unit": "<string|null>" } ]
}`;
  ...
  try {
    ...
    return { ok: true, recipe, ingredients, provider: 'openai' };
  } catch (e) {
    return {
      ok: false,
      recipe: null,
...
async function callSpoonacular(env, dish, cuisine = '', lang = 'en') {
  ...
  const searchUrl = `https://api.spoonacular.com/recipes/complexSearch?query=${encodeURIComponent(
    dish
  )}&number=1&addRecipeInformation=true&apiKey=${encodeURIComponent(apiKey)}`;
    ...
    if (!extended || !extended.length) {
      try {
        ...
        if (infoRes.ok) {
          ...
          console.log('Spoonacular info error', infoRes.status, t.slice(0, 180));
        }
      } catch (e) {
...
async function callZestful(env, lines = []) {
  ...
  try {
    ...
    const parsed = (data.results || []).map((r, i) => ({
      ...
      comment:
        r.ingredientParsed?.preparationNotes ?? r.ingredientParsed?.comment ?? r.comment ?? null,
...
async function callOFF(env, name) {
  ...
  try {
    const res = await fetch(url, { headers: { 'User-Agent': 'TummyBuddyApp/1.0' } });
...
function sumNutrition(rows = []) {
  const keys = ['energyKcal', 'protein_g', 'fat_g', 'carbs_g', 'sugar_g', 'fiber_g', 'sodium_mg'];
...
function extractMacrosFromFDC(full) {
  ...
  const deriveServing = () => {
    ...
    for (const portion of portions) {
      ...
      const desc = String(portion.portionDescription || portion.modifier || '').toLowerCase();
      const isServing =
        desc.includes('serving') || desc.includes('portion') || desc.includes('piece');
      if (!picked || isServing) {
        picked = portion;
        if (isServing) break;
      }
    ...
    if (picked) {
      ...
      if (!unit) {
        ...
        else if (picked.portionDescription) unit = String(picked.portionDescription);
        else if (picked.modifier) unit = String(picked.modifier);
      }
      if (size == null && Number.isFinite(picked.amount)) size = Number(picked.amount);
    }
  ...
  const servingGrams =
    serving?.grams && Number.isFinite(serving.grams) && serving.grams > 0 ? serving.grams : null;

  const ln = full?.labelNutrients;
  if (ln) {
    const v = (k) => ln[k]?.value ?? null;
    const perServing = empty();
    perServing.energyKcal = v('calories') != null ? Math.round(v('calories')) : null;
    perServing.protein_g = v('protein') != null ? Number(v('protein')) : null;
    perServing.fat_g = v('fat') != null ? Number(v('fat')) : null;
    perServing.carbs_g = v('carbohydrates') != null ? Number(v('carbohydrates')) : null;
    ...
    const per100g = servingGrams ? scaleMacros(perServing, 100 / servingGrams) : null;
  ...
  const carb = find(['carbohydrate, by difference', 'carbohydrate', { number: '1005' }]);
  const sug = find(['sugars, total including nlea', 'sugars, total', 'sugars', { number: '2000' }]);
...
async function callUSDAFDC(env, name) {
  ...
  const searchUrl = `https://${host}/fdc/v1/foods/search?query=${encodeURIComponent(
    name
  )}&pageSize=5&dataType=SR%20Legacy,Survey%20(FNDDS),Foundation,Branded&api_key=${key}`;
  ...
  const qualityScore = (f) => {
    ...
    const combined = [f.description, f.additionalDescriptions, f.ingredientStatement]
      .filter(Boolean)
      .join(' ');
    ...
    if (Array.isArray(f.foodNutrients) && f.foodNutrients.length > 0) score += 2;
  ...
  for (const food of foods) {
    const detail = await fetch(`https://${host}/fdc/v1/food/${food.fdcId}?api_key=${key}`, {
    ...
    const combinedDesc = [full.description, full.additionalDescriptions, full.ingredientStatement]
      .filter(Boolean)
      .join(' ');
    ...
    if (!bestMatchEvenIfProcessed && hasMacros && matches) bestMatchEvenIfProcessed = payload;
...
async function callUberForMenu(env, query, address, opts = {}, ctx) {
  ...
  const innerReq = new Request(`https://dummy.local/menu/uber-test?${params.toString()}`, {
    method: 'GET',
    headers: { accept: 'application/json' },
  });
  ...
  try {
    ...
    if (!ocrUrl && webUrlRaw) {
      ...
      if (picked) {
        ...
        return json(
          { ok: false, source: 'web_adapter', web_url: webUrlRaw, error: 'no_image_found' },
          { status: 404 }
        );
      }
    }
    ...
    if (ocrUrl && isOcrEnabled(env)) {
      try {
        ...
        return json({
          ...
          received: { query, address, ocr_url: ocrUrl, force_reanalyze: forceReanalyze, analyze },
    ...
    if (!forceLLM) {
      try {
        ...
        uberResult = uber;
        if (uber && uber.ok && Array.isArray(uber.items) && uber.items.length > 0) {
          pickedSource = 'uber';
        } else {
          pickedSource = 'none';
          notes.uber = 'empty';
    ...
    const llmCanon =
      pickedSource === 'grok' || pickedSource === 'openai' || pickedSource === 'mixed'
        ? [...(gItems || []), ...(oItems || [])]
        : [];
    ...
    if (totalPre === 0 && !ocrUrl && webUrlRaw && isOcrEnabled(env)) {
      ...
      if (picked) {
        ocrUrl = picked;
        if (debug) {
          const note = { picked_image_url: picked, note: 'picked_via_html_or_heuristic' };
    ...
    if (shouldTryOCR) {
      try {
        ...
        ocrItemsCanon = Array.isArray(ocrCandidates) ? ocrCandidates.map(toCanonFromOCR) : [];
        if (debug) notes.ocr_items = Array.isArray(ocrItemsCanon) ? ocrItemsCanon.length : 0;
        if (ocrItemsCanon.length) {
          if (pickedSource === 'none' || pickedSource === 'pending') pickedSource = 'ocr';
    ...
    if (Array.isArray(uberCanon) && !uberCanon.length) notes.uber_note = 'no_uber_items';
    if (Array.isArray(llmCanon) && !llmCanon.length) notes.llm_note = 'no_llm_items';
...
async function handleRecipeResolve(env, request, url, ctx) {
  ...
  // Provider order dials (env-driven, case-insensitive)
  const providersRecipe = (
    env.PROVIDERS_RECIPE ||
    env.provider_recipe ||
    'edamam,spoonacular,openai'
  )
    .split(',')
    .map((s) => s.trim().toLowerCase())
    .filter(Boolean);

  const providersParse = (env.PROVIDERS_PARSE || env.provider_parse || 'zestful,openai')
    .split(',')
    .map((s) => s.trim().toLowerCase())
    .filter(Boolean);

  const recipeProviderFns = {
    edamam: (env, dish, cuisine, lang) => callEdamam(env, dish, cuisine, lang),
    spoonacular: (env, dish, cuisine, lang) => callSpoonacular(env, dish, cuisine, lang),
    openai: (env, dish, cuisine, lang) => callOpenAIRecipe(env, dish, cuisine, lang),
  };
  ...
  if (cached && cached.recipe && Array.isArray(cached.ingredients) && !force) {
    ...
    if (wantShape === 'recipe_card') {
      ...
      if (isPremium && Array.isArray(ingredients) && ingredients.length) {
        ...
        for (const ing of ingredients) {
          ...
          for (const sTerm of searchTerms) {
            ...
            for (const c of comps) {
              ...
              foundCompounds.push({ name: c.name, from_ingredient: ing.name, cid: c.cid || null });
              for (const e of effs) {
        ...
        const ORG_ICON = { heart: '❤️', gut: '🦠', liver: '🧪', brain: '🧠', immune: '🛡️' };
        ...
        function humanizeOrgans(summary) {
          const tips = [];
          for (const [org, { plus = 0, minus = 0 }] of Object.entries(summary)) {
            const Org = org.charAt(0).toUpperCase() + org.slice(1);
            let line = `${Org}: `;
            if (minus > 0 && plus === 0)
              line += 'may bother sensitive tummies—consider smaller portions or swaps.';
            else if (plus > 0 && minus === 0) line += 'generally friendly in normal portions.';
            else if (plus > 0 && minus > 0) line += 'mixed signal—portion size and add-ons matter.';
        ...
        else if (totals.plus > 0 && totals.minus === 0) overall_label = 'Supportive';
        ...
        const summary_sentence =
          overall_label === 'Caution'
            ...
            : overall_label === 'Mixed'
            ? `↔️ Mixed signal — supportive for ${benefitStr || 'some organs'}, but caution for ${
                riskStr || 'others'
              }.`
        ...
        if (rc?.molecular?.compounds?.length >= 0 && !rc.molecular.gated) {
          rc.molecular_badge = `Molecular Insights: ${rc.molecular.compounds.length} compounds • ${
        ...
        rc.molecular_badge = `Molecular Insights: ${foundCompounds.length} compounds • ${
          Object.keys(organSummary).length
        } organs`;
        ...
        rc.molecular = { compounds: [], organs: {}, organ_summary: {}, gated: true };
        rc.molecular_badge = 'Molecular Insights: upgrade for details';
      }
    ...
    // Provider loop (see next block)...
    for (const p of providersRecipe) {
      const fn = recipeProviderFns[p];
      if (!fn) continue;
      try {
        out = await fn(env, dish, cuisine, lang);
      } catch (_) {}
      if (out) {
        selectedProvider = p;
        break;
  ...
  if (!cacheHit) {
    if (
      selectedProvider === 'edamam' &&
      out &&
      out.ok &&
      Array.isArray(out.items) &&
      out.items.length > 0
    ) {
      const best = out.items[0];
      const norm = normalizeEdamamRecipe(best);
      recipe = norm.recipe;
      ingredients = norm.ingredients;
      pickedSource = 'edamam';
      out = { provider: 'edamam', recipe, ingredients };
    } else if (selectedProvider === 'spoonacular' && out && out.recipe) {
      recipe = {
        name: out.recipe.title || dish,
        steps: out.recipe.instructions ? [out.recipe.instructions] : [],
        notes: out.recipe.image ? [out.recipe.image] : null,
      };
      ingredients = Array.isArray(out.ingredients) ? [...out.ingredients] : [];
      pickedSource = 'spoonacular';
      out = { provider: 'spoonacular', recipe, ingredients };
    } else if (
  ...
  const toLine = (entry) => {
    ...
    const qty = entry.qty ?? entry.quantity ?? entry.amount ?? '';
    const unit = entry.unit || entry.measure || '';
    const name = entry.name || entry.product || entry.ingredient || entry.original || '';
    const commentSrc =
      entry.comment || entry.preparation || entry.preparationNotes || entry.note || '';
    const parts = [];
    if (qty !== '' && qty !== null && qty !== undefined) parts.push(String(qty).trim());
    if (unit) parts.push(String(unit).trim());
    if (name) parts.push(String(name).trim());
    let line = parts.join(' ').trim();
    const comment = String(commentSrc || '').trim();
    if (comment) line = line ? `${line}, ${comment}` : comment;
    return line.trim();
  };

  const ingLines = Array.isArray(sourceLines) ? sourceLines.map(toLine).filter(Boolean) : [];
    ...
    if (missingIdx.length === ingLines.length || !kv) {
      ...
      if (dailyCap) {
        ...
        if (usedNow + toParse > dailyCap) {
          console.log('[parse] zestful cap guard', usedNow, '+', toParse, '>', dailyCap);
          return json(
            { ok: false, error: 'ZESTFUL_CAP_REACHED', used: usedNow, toParse, cap: dailyCap },
            { status: 429 }
          );
        }
      }
      ...
      if (Array.isArray(zest) && zest.length) {
        ...
        if (kv) {
          for (let i = 0; i < linesToParse.length; i++) {
            ...
            if (zest[i]) {
              await kv.put(k, JSON.stringify(zest[i]), { expirationTtl: 60 * 60 * 24 * 30 });
            }
          }
        }
      ...
      if (Array.isArray(zest) && zest.length) {
        for (let j = 0; j < linesToParse.length; j++) {
          ...
          if (kv && zest[j]) {
            const k = `zestful:${ingLines[i].toLowerCase()}`;
            await kv.put(k, JSON.stringify(zest[j]), { expirationTtl: 60 * 60 * 24 * 30 });
          }
        }
        await incZestfulCount(env, toParse);
      }
  ...
  if (parsed?.length) {
    ...
    Array.isArray(out?.ingredients) &&
    out.ingredients.every((x) => x && typeof x === 'object' && ('name' in x || 'original' in x))
  ) {
  ...
  if (!cacheHit && recipe && Array.isArray(ingredients) && ingredients.length) {
    await recipeCacheWrite(env, cacheKey, { recipe, ingredients, from: pickedSource });
  }
  ...
  if (classify && recipe && Array.isArray(ingredients) && ingredients.length) {
    ...
    const payload = {
      ...
      dish_name: dish,
      dish_desc:
        (Array.isArray(recipe?.notes) ? recipe.notes.join('; ') : recipe?.steps?.[0] || '') || '',
      cuisine: cuisine || '',
      ingredients: (Array.isArray(classifySource) ? classifySource : []).map((i) => {
        if (typeof i === 'string') {
    ...
    if (enqueuedOk && id) {
      await setClassifyStamp(env, stampKey, { id, dish, place_id, when: Date.now() }, 6 * 3600);
    }
  ...
  if (wantShape === 'recipe_card') {
    ...
    if (isPremium && Array.isArray(ingredients) && ingredients.length) {
      ...
      rc.molecular = { compounds: foundCompounds, organs: organMap, organ_summary: organSummary };
    } else {
      rc.molecular = { compounds: [], organs: {}, organ_summary: {}, gated: true };
    }
...
async function fetchMenuFromUberEats(
  ...
  async function postJob() {
    ...
    const body = { scraper: { maxRows, query, address, locale: 'en-US', page: 1 } };
    const res = await fetch(url, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
  ...
  while (attempt < 3) {
    ...
    if (!isJSON) {
      ...
      throw new Error(
        `UberEats non-JSON response (${res.status} ${res.statusText}): ${text.slice(0, 200)}...`
      );
    }

    parsed = await res.json();
    if (!res.ok)
      throw new Error(`UberEats error: ${res.status} ${res.statusText} ${JSON.stringify(parsed)}`);
    break;
  }

  if (!parsed) throw new Error('UberEats: failed to get JSON after retries.');

  const hasCandidates =
    (Array.isArray(parsed?.data?.results) && parsed.data.results.length) ||
    (Array.isArray(parsed?.results) && parsed.results.length) ||
    (Array.isArray(parsed?.data?.data?.results) && parsed.data.data.results.length);
  ...
  const jobId = parsed.id || parsed?.data?.id || parsed?.job?.id;
  const state = parsed.state || parsed?.data?.state || parsed?.job?.state;

  if (jobId && state && state !== 'completed') {
    for (let i = 0; i < 5; i++) {
      await sleep(1500);
      const res = await getById(jobId);
      const ctype = (res.headers.get('content-type') || '').toLowerCase();
      if (!ctype.includes('application/json')) {
        const t = await res.text().catch(() => '');
        if ([502, 503, 504].includes(res.status)) continue;
        throw new Error(
          `UberEats poll non-JSON (${res.status} ${res.statusText}): ${t.slice(0, 200)}...`
        );
      }
      const j = await res.json();
      const s = j.state || j?.data?.state || j?.job?.state;
...
async function postJobByLocation({ query, lat, lng, radius = 6000, maxRows = 25 }, env) {
  const host = env.UBER_RAPID_HOST || 'uber-eats-scraper-api.p.rapidapi.com';
  ...
  async function attemptOnce(url) {
    ...
    if (res.status === 429) {
      ...
      await sleep(waitMs);
      throw new Error(`RETRYABLE_429:${Math.max(0, Math.floor(waitMs / 1000))}`);
    }
  ...
  throw new Error(lastErr ? String(lastErr) : 'UberEats: no working location endpoint found.');
}

// ---- UBER EATS: SEARCH BY GPS (fallback when job/location 404s) ----
async function searchNearbyCandidates({ query, lat, lng, radius = 6000, maxRows = 25 }, env) {
  const host = env.UBER_RAPID_HOST || 'uber-eats-scraper-api.p.rapidapi.com';
  const key = env.RAPIDAPI_KEY;
  if (!key) throw new Error('Missing RAPIDAPI_KEY');
  if (!host) throw new Error('Missing UBER_RAPID_HOST');
  ...
  for (const p of candidatePaths) {
    ...
    for (let i = 0; i < 4; i++) {
      try {
          ...
          for (const it of arr) {
            flat.push({
              id:
                ...
                it?.url,
              title:
                it?.title || it?.name || it?.displayName || it?.storeName || it?.restaurantName,
              raw: it,
            });
          }
        }
        ...
        return { ok: true, pathTried: p, count: clean.length, candidates: clean, raw: data };
      } catch (err) {
        const msg = String(err?.message || err);
        const retryable = msg.includes('RETRYABLE_429') || /UberEats (502|503|504)/.test(msg);
        if (!retryable) break;
...
function isUSRow(row) {
  const country = (row?.location?.country || row?.location?.geo?.country || row?.country || '')
    .toString()
    .toLowerCase();
  const region = (row?.location?.region || row?.location?.geo?.region || row?.region || '')
    .toString()
    .toUpperCase();
...
async function pollUberJobUntilDone({ jobId, env, maxTries = 12 }) {
  ...
  for (let i = 0; i < maxTries; i++) {
    const res = await fetch(`https://${host}/api/job/${encodeURIComponent(jobId)}`, {
      method: 'GET',
...
async function runAddressJobRaw(
  { query = 'seafood', address = 'South Miami, FL, USA', maxRows = 3, locale = 'en-US', page = 1 },
  env
) {
  const host = env.UBER_RAPID_HOST || 'uber-eats-scraper-api.p.rapidapi.com';
  const key = env.RAPIDAPI_KEY || '';
  const body = {
    scraper: { maxRows: Number(maxRows) || 3, query, address, locale, page: Number(page) || 1 },
  };
...
function explainRestaurantChoices({ rows, query, limit = 10 }) {
  ...
  function scoreRow(r) {
    ...
    for (const t of qtoks) if (ttoks.has(t)) overlap++;
    const ratio = overlap / Math.max(1, qtoks.size);
    return { score: Math.round(60 * ratio), kind: 'token-overlap', overlap, denom: qtoks.size };
  }
...
async function refreshShardIndex(env) {
  ...
  if (!base || !key) throw new Error('Missing LEXICON_API_URL or LEXICON_API_KEY');
  ...
  if (!text) return { ok: false, error: 'No index endpoint succeeded', tried: statusTried };

  const js = parseJsonSafe(text, null);
  if (!js) return { ok: false, error: 'Index JSON invalid', tried: statusTried };
        ...
        try {
          ...
          if (res.ok) {
            ...
            if (env.LEXICON_CACHE && shard) {
              try {
                await env.LEXICON_CACHE.put(key, JSON.stringify(shard), { expirationTtl: 86400 });
              } catch {}
            }
          }
        } catch (err) {
          console.log('[lexicon] shard fetch failed', name, err?.message || err);
        }
      }
    }
...
const FALLBACK_INGREDIENTS = [
  ...
  { term: 'noodles', canonical: 'noodles', classes: ['gluten'], tags: ['wheat'] },
  { term: 'flour', canonical: 'wheat flour', classes: ['gluten'], tags: ['wheat', 'flour'] },
  { term: 'wheat', canonical: 'wheat', classes: ['gluten'], tags: ['wheat'] },
  { term: 'cream', canonical: 'cream', classes: ['dairy'], tags: ['milk', 'dairy'] },
  { term: 'butter', canonical: 'butter', classes: ['dairy'], tags: ['milk', 'dairy'] },
  {
    term: 'parmesan',
    canonical: 'parmesan',
    classes: ['dairy'],
  ...
  { term: 'cheese', canonical: 'cheese', classes: ['dairy'], tags: ['milk', 'dairy'] },
  { term: 'garlic', canonical: 'garlic', classes: ['allium'], tags: ['garlic'] },
...
function tidyIngredientHits(hits, limit = 25) {
  ...
  const out = Array.from(byCanon.values()).sort((a, b) => {
    ...
    return (a.canonical || a.term || '').localeCompare(b.canonical || b.term || '');
  });
...
function scoreHit(h) {
  ...
  if (typeof h.weight === 'number') s += Math.max(-1, Math.min(1, h.weight - 0.5));
  return s;
}
  ...
  async queue(batch, env, ctx) {
    for (const msg of batch.messages) {
      ...
      try {
        ...
        if (job?.force_reanalyze) dummyUrl.searchParams.set('force_reanalyze', '1');
        const cachedResult = await maybeReturnCachedResult(new Request(dummyUrl.toString()), env, {
        ...
        try {
          ...
          lexiconResult = { error: `Lexicon failed: ${e?.message || String(e)}` };
        ...
        const tummy_barometer = scoring.tummy_barometer;
        const flags = {
          ...legacyFlags,
          allergens: scoring.flags.allergens,
          fodmap: scoring.flags.fodmap,
        };
        const sentences = buildHumanSentences(flags, tummy_barometer);

        // ---- Secondary (fallback) call: RapidAPI mirror (simple counter) ----
        let rapidResult = null;
        try {
          rapidResult = await callRapid(env, job?.dish_name ?? '', job?.address || '');
          console.log('[RAPID] success');
        } catch (e) {
          rapidResult = { error: `RapidAPI failed: ${e?.message || String(e)}` };
        ...
        try {
          ...
          if (lexiconResult?.ok) {
            ...
            await bumpDaily(env, { lex_ok: isLive ? 0 : 1, lex_live: isLive ? 1 : 0 });
          ...
          console.log('[STATS] bump error (non-fatal):', e?.message || String(e));
        }
        ...
        // KV bookmarks
        if (env.LEXICON_CACHE) {
          await env.LEXICON_CACHE.put('last_job_id', id, { expirationTtl: 3600 });
          await env.LEXICON_CACHE.put('last_job_ts', String(Date.now()), { expirationTtl: 3600 });
        }

        // D1 log
        if (env.D1_DB) {
          try {
            await env.D1_DB.prepare('INSERT INTO logs (kind, ref, created_at) VALUES (?, ?, ?)')
              .bind('dish_job', key, Date.now())
              .run();
          } catch (e2) {
            console.log('[QUEUE] D1 log error (non-fatal):', e2?.message || String(e2));
          }
        }
  ...
  async fetch(request, env, ctx) {
    ...
    if (request.method === 'POST' && pathname === '/enqueue') {
      ...
      if (!env.ANALYSIS_QUEUE)
        return jsonResponse({ ok: false, error: 'ANALYSIS_QUEUE not bound' }, 500);
    ...
    if (pathname === '/meta') {
      const ctx = { served_at: new Date().toISOString(), version: getVersion(env) };
      ...
      return jsonResponseWithTB(withBodyAnalytics(body, ctx, rid, { endpoint: 'meta' }), 200, {
        ctx,
        rid,
        source: 'meta',
        cache: '',
    ...
    if (pathname === '/debug/status') {
      const ctx = { served_at: new Date().toISOString(), version: getVersion(env) };
      const status = (await readStatusKV(env)) || { updated_at: null, counts: {} };
      const rid = newRequestId();
      const body = withBodyAnalytics({ ok: true, status }, ctx, rid, { endpoint: 'debug-status' });
      return jsonResponseWithTB(body, 200, { ctx, rid, source: 'debug-status' });
    }
    ...
    if (pathname === '/debug/fdc-cache') {
      if (!env?.R2_BUCKET)
        return json({ ok: false, error: 'R2_BUCKET not bound' }, { status: 400 });
      const u = new URL(request.url);
    ...
    if (pathname === '/debug/off') {
      const u = new URL(request.url);
      const name = (u.searchParams.get('name') || '').trim();
      if (!name) return json({ ok: false, error: 'missing name' }, { status: 400 });
    ...
    if (pathname === '/debug/job') {
      ...
      if (!env.R2_BUCKET) return jsonResponse({ ok: false, error: 'R2 not bound' }, 500);
      ...
      if (!obj) return jsonResponse({ ok: false, error: 'not_found', key: keyNew }, 404);
      const body = await obj.text();
    ...
    if (pathname === '/debug/ping-lexicon') {
      ...
      const tryXKey = await fetch(urlToCall, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'x-api-key': key, 'Accept-Language': 'en' },
        body: JSON.stringify({
    ...
    if (pathname === '/debug/read-kv-shard') {
      ...
      if (!env.LEXICON_CACHE) return jsonResponse({ ok: false, error: 'KV not bound' }, 500);

      const key = `shards/${name}.json`;
      const raw = await env.LEXICON_CACHE.get(key);
      if (!raw) return jsonResponse({ ok: false, error: 'not_in_kv', key }, 404);
    ...
    if (pathname === '/debug/warm-lexicon') {
      const list = lc(searchParams.get('names') || '')
        ...
        .map((s) => s.trim())
        .filter(Boolean);
      if (!list.length) return jsonResponse({ ok: false, error: 'missing names' }, 400);
      ...
      for (const name of list) {
        ...
        try {
          ...
          if (env.LEXICON_CACHE) {
            await env.LEXICON_CACHE.put(`shards/${name}.json`, text, { expirationTtl: 86400 });
          }
          out.push({
            shard: name,
            url,
          ...
          out.push({ shard: name, url, status: 0, ok: false, error: e?.message || String(e) });
        }
      }
      ...
      try {
        ...
        for (const name of names) {
          ...
          try {
            ...
            warmed.push({ shard: name, url, status: 0, ok: false, error: e?.message || String(e) });
          }
        }

        return jsonResponse({ ok: true, index_refresh: idx, warmed_count: warmed.length, warmed });
      } catch (e) {
    ...
    if (pathname.startsWith('/results/')) {
      ...
      if (request.method === 'GET') {
        ...
        const obj = await env.R2_BUCKET.get(key);
        if (!obj) {
          return new Response(JSON.stringify({ ok: false, error: 'not_found', key }), {
            status: 404,
            headers: { ...CORS, 'content-type': 'application/json' },
          });
        }
    ...
    if (pathname === '/debug/rapid-job') {
      ...
      const body = { scraper: { maxRows: max, query: q, address: addr, locale, page } };
    ...
    if (pathname === '/debug/r2-get') {
      ...
      const obj = await env.R2_BUCKET.get(key);
      if (!obj) return jsonResponse({ ok: false, error: 'not_found', key }, 404);
    ...
    if (pathname === '/menu/extract') return handleMenuExtract(env, request, url, ctx);
    if (pathname === '/recipe/resolve') return handleRecipeResolve(env, request, url, ctx);
    ...
    if (pathname === '/menu/uber-test' && request.method === 'GET') {
      ...
      try {
        ...
        trace.host = trace.host || env.UBER_RAPID_HOST || 'uber-eats-scraper-api.p.rapidapi.com'; // [38.9]
        ...
        try {
          ...
          cacheAgeSec = null;
          if (cached?.savedAt) {
            cacheAgeSec = Math.max(0, Math.floor((Date.now() - Date.parse(cached.savedAt)) / 1000));
          }
          // [38.3] age warning for cached data (>20h considered stale-ish)
          if (cached?.savedAt) {
            const ageSec = Math.max(0, (Date.now() - Date.parse(cached.savedAt)) / 1000);
            if (ageSec > 20 * 3600) setWarn('Cached data is older than ~20 hours (may be stale).');
          }
                  ...
                  withBodyAnalytics(
                    {
                      ...
                      cache: cacheStatus,
                      cache_age_seconds: cacheAgeSec,
                      data: { query, address, forceUS, items: flattenedItems.slice(0, maxRows) },
                      enqueued,
                      ...warnPart(),
                    },
                    ctx,
        ...
        if (addressRaw) {
          ...
          if (forceUSFlag && !/usa|united states/i.test(address)) address = `${addressRaw}, USA`;

          const job = await postJobByAddress({ query, address, maxRows, locale, page }, env);
          ...
          if (job?.immediate) {
            ...
            if (debug === '1') {
              ...
              await bumpStatusKV(env, { debug: 1 });
              const preview = buildDebugPreview(job?.raw || {}, env, rowsUS, titles);
            ...
            if (debug === 'why') {
              const exp = explainRestaurantChoices({ rows: rowsUS, query, limit: 10 });
              await bumpStatusKV(env, { debug: 1 });
              return respondTB(
                withBodyAnalytics(
                  {
            ...
            if (!chosen) {
              await bumpStatusKV(env, { errors_4xx: 1 });
              return notFoundCandidates(ctx, rid, trace, { query, address: addressRaw });
            }
          ...
          if (debug === '1') {
            ...
            const titles = (Array.isArray(rowsUS) ? rowsUS : [])
              ...
              .filter(Boolean);
            await bumpStatusKV(env, { debug: 1 });
            const preview = buildDebugPreview(finished || {}, env, rowsUS, titles);
            return respondTB(withBodyAnalytics(preview, ctx, rid, trace), 200);
          }
        ...
        if (lat && lng) {
          try {
            ...
            if (debug === 'titles') {
              return respondTB(
                withBodyAnalytics(
                  { ok: true, source: 'location-job', count: titles.length, titles, ...warnPart() },
                  ctx,
                  rid,
                  trace
                ),
            ...
            if (msg.includes('HARD_404')) {
              ...
              setWarn('Used GPS search fallback (vendor location job unavailable).');
            ...
            if (/\b429\b/.test(msg)) {
              ...
              return rateLimitResponse(ctx, rid, trace, secs > 0 ? secs : 30, 'upstream-failure');
            }
            await bumpStatusKV(env, { errors_5xx: 1 });
            return errorResponseWith(
              {
        ...
        if (debug === 'rank') {
          ...
          return respondTB(
            withBodyAnalytics(
              { ok: true, source: 'live', cache: cacheStatus, preview, ...warnPart() },
              ctx,
              rid,
              trace
            ),
        ...
        await bumpStatusKV(env, { errors_5xx: 1 });
        const hint =
          upstreamStatus === 429 ? 'Please retry in ~30–60 seconds.' : 'Please try again shortly.';
    ...
    if (pathname === '/menu/search') {
      ...
      const innerReq = new Request(new URL(`/menu/uber-test?${params.toString()}`, url), {
        method: 'GET',
        headers: { accept: 'application/json' },
      });
      const innerRes = await _worker_impl.fetch(innerReq, env, ctx);
      if (!innerRes || !innerRes.ok) {
        ...
        const hint =
          status === 429 ? 'Please retry in ~30–60 seconds.' : 'Please try again shortly.';
    ...
    if (pathname === '/molecular/analyze') {
      ...
      try {
        ...
        const body = {
          ok: true,
          query: { ingredient: byIngredient || null, compound: byCompound || null },
    ...
    if (pathname === '/molecular/compound') {
      ...
      try {
        ...
        if (!cid && !name) {
          return new Response(
            JSON.stringify({
              ...
              examples: ['/molecular/compound?cid=969516', '/molecular/compound?name=curcumin'],
            }),
            {
              status: 400,
              headers: { 'content-type': 'application/json', 'access-control-allow-origin': '*' },
        ...
        if (!compRow) {
          return new Response(JSON.stringify({ ok: false, error: 'compound_not_found' }), {
            status: 404,
            headers: { 'content-type': 'application/json', 'access-control-allow-origin': '*' },
          });
        }
        ...
        for (const e of effects) {
          const k = e.organ || 'unknown';
          if (!organ_summary[k]) organ_summary[k] = { plus: 0, minus: 0, neutral: 0 };
    ...
    if (pathname === '/debug/kv-tier') {
      ...
      try {
        ...
        return new Response(JSON.stringify({ ok: true, user_id: uid, key, value: val ?? null }), {
          status: 200,
          headers: { 'content-type': 'application/json', 'access-control-allow-origin': '*' },
        });
      } catch (e) {
    ...
    if (pathname === '/debug/kv-set') {
      ...
      if (!uid || !tier) {
        return new Response(JSON.stringify({ ok: false, error: 'missing user_id or tier' }), {
          status: 400,
          headers: { 'content-type': 'application/json', 'access-control-allow-origin': '*' },
        });
      }
    ...
    if (pathname === '/schema/examples') {
      ...
      const RecipeCard = {
        ...
        molecular_human: {
          organ_tips: ['Gut: may bother sensitive tummies—consider smaller portions or swaps.'],
        },
      ...
      const payload = { ok: true, schemas: { MolecularDrawer, RecipeCard, InsightsFeed } };
    ...
    if (pathname === '/insights/feed') {
      ...
      if (env.R2_BUCKET) {
        ...
        const objs = (listing.objects || [])
          .map((o) => ({ key: o.key, uploaded: o.uploaded ? new Date(o.uploaded).getTime() : 0 }))
      ...
      const filtered = organParam
        ...
        : items;
      const ORDER = { Supportive: 1, Mixed: 2, Caution: 3, Avoid: 4, Unknown: 0 };
      const minParam = (url.searchParams.get('min_sentiment') || '').toLowerCase();
      const MIN = { supportive: 1, mixed: 2, caution: 3, avoid: 4 }[minParam] ?? 0;
      const filteredItems = filtered.filter((it) => (ORDER[it.sentiment] || 0) >= MIN);
      ...
      for (const it of sliced) {
        if (summary_counts[it.sentiment] != null) summary_counts[it.sentiment]++;
      }
    ...
    if (pathname === '/insights/debug/r2-list') {
      ...
      const listing = await env.R2_BUCKET.list({ prefix, limit, include: ['httpMetadata'] });
      const items = (listing.objects || []).map((o) => ({
        key: o.key,
        size: o.size,
        uploaded: o.uploaded ? new Date(o.uploaded).toISOString() : null,
      }));
      return new Response(JSON.stringify({ ok: true, count: items.length, items }, null, 2), {
        status: 200,
        headers: {
          'content-type': 'application/json; charset=utf-8',
          'access-control-allow-origin': '*',
    ...
    if (pathname === '/insights/debug/sample') {
      ...
      const key =
        url.searchParams.get('key') || 'results/0032d323-560f-469f-a496-195812a9efd4.json';
      const obj = await env.R2_BUCKET.get(key);
      if (!obj) {
        return new Response(JSON.stringify({ ok: false, error: 'not_found', key }), {
          status: 404,
    ...
    if (pathname === '/insights/debug/aggregate') {
      ...
      const counts = { Supportive: 0, Mixed: 0, Caution: 0, Avoid: 0, Unknown: 0 };
      const sample = [];
      for (const obj of listing.objects || []) {
        ...
        try {
          ...
          if (sample.length < 5) {
            sample.push({ key: obj.key, label, score: js?.tummy_barometer?.score ?? null });
          }
        } catch {
    ...
    if (pathname === '/debug/spoonacular') {
      ...
      try {
        ...
        const endpoint = `https://api.spoonacular.com/recipes/complexSearch?query=${encodeURIComponent(
          dish
        )}&number=1&addRecipeInformation=true&apiKey=${encodeURIComponent(apiKey)}`;
    ...
    if (pathname === '/debug/zestful') {
      ...
      try {
        const spoon = await callSpoonacular(env, dish, null, 'en');
    ...
    if (pathname === '/debug/zestful-raw') {
      ...
      try {
        const sp = await callSpoonacular(env, dish, null, 'en');
        const lines = Array.isArray(sp?.ingredients) ? sp.ingredients.slice(0, 10) : [];
        if (!lines.length) return json({ ok: false, note: 'no lines from provider', host });
...
function limitsSnapshot({ maxRows, radius }) {
  ...
  return {
    maxRows: { requested: reqMax, effective: effectiveMax, min: 1, max: 50, default: 15 },
    radius: { requested: reqRad, effective: effectiveRad, min: 500, max: 25000, default: 5000 },
...
async function getZestfulCount(env) {
  ...
  try {
    ...
    console.log('[parse] zestful counter read error:', err?.message || String(err));
    return 0;
  }
}
...
async function incZestfulCount(env, linesCount = 0) {
  ...
  try {
    ...
    console.log('[parse] zestful counter put error:', err?.message || String(err));
  }
...
async function getShardFromKV(env, name) {
  ...
  if (!js || !Array.isArray(js.entries)) return { ok: false, name, reason: 'bad_format' };
...
const toScore = (raw, maxRaw = RISK.maxRaw) => Math.round(clamp01(raw / maxRaw) * 100);
const labelFor = (score) => (score <= 39 ? 'Avoid' : score <= 69 ? 'Caution' : 'Likely OK');

function extractAllergenKeys(hit) {
  return inferAllergensFromClassesTags(hit).map((x) => x.trim().toLowerCase().replace(/\s+/g, '_'));
}
...
function inferAllergensFromClassesTags(hit) {
  ...
  if (tags.includes('wheat') || /wheat|semolina|breadcrumbs|pasta|flour/.test(canon))
    out.add('wheat');
  if (classes.includes('shellfish') || tags.includes('shellfish')) out.add('shellfish');
...
function scoreDishFromHits(hits) {
  ...
  for (const k of Array.from(allergenSet).sort()) {
    ...
    if (k === 'milk' && worstFodmap === 'low') {
      w = Math.round(w * 0.6);
      reasons.push({ kind: 'allergen', key: k, weight: w, note: 'trace/low-lactose dairy' });
    } else {
...
function buildHumanSentences(flags, tummy_barometer) {
  ...
  if (f === 'high') out.push('FODMAP level appears high; sensitive users should avoid or confirm.');
  else if (f === 'medium') out.push('FODMAP level appears medium; portion size may matter.');
  else if (f === 'low') out.push('FODMAP level appears low; small portions are often tolerated.');
  ...
  if (typeof tummy_barometer?.score === 'number' && tummy_barometer?.label) {
    out.push(`Overall: ${tummy_barometer.label} (score ${tummy_barometer.score}).`);
  }
...
async function bumpDaily(
  env,
  { jobs = 0, lex_ok = 0, lex_live = 0, lex_err = 0, rap_ok = 0, rap_err = 0 } = {}
) {
  if (!env.D1_DB) return;
  const day = dayStrUTC();
...
function buildLexiconAnalyzeCandidates(base) {
  ...
  return [`${b}/v1/analyze`, `${b}/analyze`, `${b}/api/analyze`, `${b}/v1/lexicon/analyze`];
}
...
async function callLexicon(env, text, lang = 'en') {
  const base = normalizeBase(env.LEXICON_API_URL);
  const key = env.LEXICON_API_KEY;
  if (!base || !key) throw new Error('LEXICON_API_URL or LEXICON_API_KEY missing');
  ...
  const headersVariants = [
    ...
    (k) => ({ 'x-api-key': k, 'Content-Type': 'application/json', 'Accept-Language': lang }),
  ];
...
async function lexiconAnalyzeViaShards(env, text, lang = 'en') {
  ...
  const names = index ? pickIngredientShardNamesFromIndex(index) : STATIC_INGREDIENT_SHARDS;
...
function errorResponseWith(errBody, status = 400, envOrCtx, meta = {}, request_id = null) {
  const hasCtx =
    envOrCtx && typeof envOrCtx === 'object' && 'served_at' in envOrCtx && 'version' in envOrCtx;
...
function handleScheduled(controller, env, ctx) {
  return _worker_impl.scheduled ? _worker_impl.scheduled(controller, env, ctx) : undefined;
}
