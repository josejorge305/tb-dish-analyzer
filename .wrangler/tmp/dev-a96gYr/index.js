...
// .wrangler/tmp/bundle-Vi314h/checked-fetch.js
...
`;
var fatSecretCachedToken = null;
var fatSecretTokenExpiresAt = null;
...
__name(fetchImageAsBase64, "fetchImageAsBase64");
var DEFAULT_SERVINGS_BY_CATEGORY = {
  "Pasta & Pizza": 1.3,
...
__name(isDietTitle, "isDietTitle");
...
async function getFatSecretAccessToken(env) {
  ...
  const now = Date.now();
  if (fatSecretCachedToken && fatSecretTokenExpiresAt && now < fatSecretTokenExpiresAt - 6e4) {
  ...
  fatSecretTokenExpiresAt = Date.now() + expiresInSec * 1e3;
  return fatSecretCachedToken;
}
__name(getFatSecretAccessToken, "getFatSecretAccessToken");
...
__name(callFatSecretImageRecognition, "callFatSecretImageRecognition");
...
function extractMenuItemsFromUber(raw, queryText = "") {
  ...
  __name(extractCaloriesFromText, "extractCaloriesFromText");
  ...
  function makeItem(mi, sectionName, restaurantName) {
    ...
    const restaurantCalories = (mi?.nutrition && typeof mi.nutrition.calories === "number" ? mi.nutrition.calories : null) ?? (typeof mi?.calories === "number" ? mi.calories : null) ?? extractCaloriesFromText(price_display) ?? extractCaloriesFromText(mi?.itemDescription || mi?.description || "") ?? null;
    return {
...
- Estimate primary cooking method for the main visible item(s):
  - primary: "fried" | "deep_fried" | "baked" | "grilled" | "broiled" |
             "boiled" | "steamed" | "raw" | "roasted" | "saut\xE9ed" | "mixed" | "unknown"
...
`.trim();
  ...
  try {
    ...
    const plateComponentsRaw = Array.isArray(parsed?.plate_components) ? parsed.plate_components : [];
    const pf = typeof portion.portionFactor === "number" && isFinite(portion.portionFactor) ? portion.portionFactor : typeof portion.servings_on_plate === "number" && isFinite(portion.servings_on_plate) ? portion.servings_on_plate : 1;
    const conf = typeof portion.confidence === "number" && isFinite(portion.confidence) ? portion.confidence : 0;
    const reason = typeof portion.reason === "string" ? portion.reason : "No portion reason provided.";
    let plate_components = plateComponentsRaw.filter((c) => {
      if (!c || typeof c !== "object") return false;
      const conf2 = typeof c.confidence === "number" ? c.confidence : 0;
      return conf2 >= 0.35;
    }).map((c) => {
      const role = typeof c.role === "string" && c.role ? c.role : "unknown";
      const category = typeof c.category === "string" && c.category ? c.category : "other";
      const label = typeof c.label === "string" && c.label || typeof c.component === "string" && c.component || typeof c.name === "string" && c.name || "Component";
      const confidence = typeof c.confidence === "number" ? c.confidence : 0;
      const area_ratio = typeof c.area_ratio === "number" && c.area_ratio > 0 ? c.area_ratio : 0;
      return {
        role,
        category,
        label,
    ...
    let sumAreas = plate_components.reduce(
      (sum, c) => sum + (typeof c.area_ratio === "number" && c.area_ratio > 0 ? c.area_ratio : 0),
      0
    );
    if (plate_components.length > 0) {
      if (!sumAreas || sumAreas <= 0) {
        ...
        plate_components = plate_components.map((c) => {
          const raw = typeof c.area_ratio === "number" && c.area_ratio > 0 ? c.area_ratio : 0;
          return {
            ...c,
    ...
    const insights = {
      portion: {
        servings_on_plate: typeof portion.servings_on_plate === "number" && isFinite(portion.servings_on_plate) ? portion.servings_on_plate : pf,
        portionFactor: pf,
        confidence: conf,
        reason
      },
      visual_ingredients: Array.isArray(parsed.visual_ingredients) && parsed.visual_ingredients.length > 0 ? parsed.visual_ingredients : [],
      visual_cooking_method: parsed.visual_cooking_method && typeof parsed.visual_cooking_method === "object" ? parsed.visual_cooking_method : {
        primary: "unknown",
        secondary: [],
        confidence: 0,
        reason: ""
      },
      visual_lifestyle_cues: parsed.visual_lifestyle_cues && typeof parsed.visual_lifestyle_cues === "object" ? parsed.visual_lifestyle_cues : {
...
async function resolveRecipeWithCache(env, {
  ...
  __name(resolveFromProviders, "resolveFromProviders");
  ...
  const cachedTitle = out && out.recipe && (out.recipe.title || out.recipe.name) || "";
  const dishLower = dish.toLowerCase();
  if (cacheHit && cachedTitle && isDietTitle(cachedTitle) && !isDietTitle(dishLower)) {
    cacheHit = false;
    out = null;
    recipe = null;
    ingredients = [];
...
async function extractMenuGateway(env, { placeId, url, restaurantName, address, lat, lng }) {
  ...
  const dishes = (uber.items || []).map((it, idx) => {
    ...
    const numericRestaurantCalories = (typeof it.restaurantCalories === "number" ? it.restaurantCalories : null) ?? (raw?.nutrition && typeof raw.nutrition.calories === "number" ? raw.nutrition.calories : null) ?? (typeof it.calories === "number" ? it.calories : null) ?? extractCaloriesFromText(it.price_display) ?? extractCaloriesFromText(it.priceText) ?? extractCaloriesFromText(it.description || raw.description || "") ?? null;
    return {
      id: `canon-${idx + 1}`,
...
async function runDishAnalysis(env, body, ctx) {
  const correlationId = body && body.correlationId || body && body.cid || null;
  let debug = {};
  const useParallel = env && env.PARALLEL_LLM === "true";
  const lang = body.lang || body.language || "en";
  let devFlag = body.dev === true || body.dev === 1 || body.dev === "1" || false;
  ...
  ) ? body.selection_component_ids : null;
  ...
  devFlag = devFlag || recipeResult?.devFlag || recipeResult?.out?.devFlag || false;
  ...
  if (restaurantCalories != null && !Number.isNaN(Number(restaurantCalories))) {
    ...
    if (!finalNutritionSummary) {
      ...
      const baseKcal = typeof finalNutritionSummary.energyKcal === "number" ? finalNutritionSummary.energyKcal : 0;
  ...
  if (useParallel) {
    ...
    const [allergenSettled, organsSettled, nutritionSettled] = await Promise.allSettled(promises);
  ...
  if (allergenMiniResult && allergenMiniResult.ok && allergenMiniResult.data && typeof allergenMiniResult.data === "object") {
    ...
    try {
      ...
      if (Array.isArray(componentAllergensRaw) && componentAllergensRaw.length > 0 && plateComponents && plateComponents.length > 0) {
        allergen_breakdown = componentAllergensRaw.map((compEntry, idx) => {
          const pc = plateComponents[idx] || {};
          const label = compEntry?.component_label || pc.label || pc.component || pc.name || `Component ${idx + 1}`;
  ...
  organs = null;
  if (organsLLMResult && organsLLMResult.ok && organsLLMResult.data && typeof organsLLMResult.data === "object") {
  ...
  if (organs && organs.ok) {
    ...
    let summarySentences2 = [];
    try {
      if (summary && summary.tummyBarometer) {
        summarySentences2 = buildHumanSentences(
          organs.flags || {},
        ...
        summary.sentences = [...summarySentences2, ...organSentences];
      }
    } catch {
    }
    const edamamHealthLabels2 = getEdamamHealthLabelsFromRecipe(recipeResult);
    if (summary) {
      summary.edamamLabels = edamamHealthLabels2;
    }
  ...
  if (devFlag && dishImageUrl) {
    try {
      ...
      debug.fatsecret_image_error = "exception:" + String(e && e.message ? e.message : e);
    }
  }
  ...
  if (portionVisionDebug && portionVisionDebug.ok && portionVisionDebug.insights) {
  ...
  try {
    ...
    const recipeProvider = recipeResult?.out?.provider ?? recipeResult?.source ?? recipeResult?.responseSource ?? null;
    const recipeOut = recipeResult?.out;
    const hasYield = recipeOut && typeof recipeOut.yield === "number" && recipeOut.yield > 0;
    const hasServings = recipeOut && typeof recipeOut.servings === "number" && recipeOut.servings > 0;
    if (recipeProvider === "openai" && !hasYield && !hasServings) {
      openaiServingsDivisor = 4;
    }
    if (openaiServingsDivisor > 1 && finalNutritionSummary && typeof finalNutritionSummary === "object") {
      finalNutritionSummary = {
        energyKcal: typeof finalNutritionSummary.energyKcal === "number" ? finalNutritionSummary.energyKcal / openaiServingsDivisor : finalNutritionSummary.energyKcal,
        protein_g: typeof finalNutritionSummary.protein_g === "number" ? finalNutritionSummary.protein_g / openaiServingsDivisor : finalNutritionSummary.protein_g,
        fat_g: typeof finalNutritionSummary.fat_g === "number" ? finalNutritionSummary.fat_g / openaiServingsDivisor : finalNutritionSummary.fat_g,
        carbs_g: typeof finalNutritionSummary.carbs_g === "number" ? finalNutritionSummary.carbs_g / openaiServingsDivisor : finalNutritionSummary.carbs_g,
        sugar_g: typeof finalNutritionSummary.sugar_g === "number" ? finalNutritionSummary.sugar_g / openaiServingsDivisor : finalNutritionSummary.sugar_g,
        fiber_g: typeof finalNutritionSummary.fiber_g === "number" ? finalNutritionSummary.fiber_g / openaiServingsDivisor : finalNutritionSummary.fiber_g,
        sodium_mg: typeof finalNutritionSummary.sodium_mg === "number" ? finalNutritionSummary.sodium_mg / openaiServingsDivisor : finalNutritionSummary.sodium_mg
      };
  ...
  try {
    ...
    const baseKcal = finalNutritionSummary && typeof finalNutritionSummary.energyKcal === "number" ? finalNutritionSummary.energyKcal : null;
    const hasRestaurantCalories = typeof restaurantCalories === "number" && restaurantCalories > 0;
    if (!hasRestaurantCalories && baseKcal != null && baseKcal > 2e3) {
      nutritionMultiServingsDivisor = 4;
    }
    if (nutritionMultiServingsDivisor > 1 && finalNutritionSummary && typeof finalNutritionSummary === "object") {
      const d = nutritionMultiServingsDivisor;
      finalNutritionSummary = {
        energyKcal: typeof finalNutritionSummary.energyKcal === "number" ? finalNutritionSummary.energyKcal / d : finalNutritionSummary.energyKcal,
        protein_g: typeof finalNutritionSummary.protein_g === "number" ? finalNutritionSummary.protein_g / d : finalNutritionSummary.protein_g,
        fat_g: typeof finalNutritionSummary.fat_g === "number" ? finalNutritionSummary.fat_g / d : finalNutritionSummary.fat_g,
        carbs_g: typeof finalNutritionSummary.carbs_g === "number" ? finalNutritionSummary.carbs_g / d : finalNutritionSummary.carbs_g,
        sugar_g: typeof finalNutritionSummary.sugar_g === "number" ? finalNutritionSummary.sugar_g / d : finalNutritionSummary.sugar_g,
        fiber_g: typeof finalNutritionSummary.fiber_g === "number" ? finalNutritionSummary.fiber_g / d : finalNutritionSummary.fiber_g,
        sodium_mg: typeof finalNutritionSummary.sodium_mg === "number" ? finalNutritionSummary.sodium_mg / d : finalNutritionSummary.sodium_mg
      };
      if (debug) {
  ...
  if (dishImageUrl && !restaurantCalories && portionVisionDebug && portionVisionDebug.ok && portionVisionDebug.source === "portion_vision_openai") {
    ...
    if (typeof portionInsights.servings_on_plate === "number" && isFinite(portionInsights.servings_on_plate)) {
      pfCandidate = portionInsights.servings_on_plate;
    } else if (typeof portionInsights.portionFactor === "number" && isFinite(portionInsights.portionFactor)) {
      pfCandidate = portionInsights.portionFactor;
    } else if (typeof portionVisionDebug.portionFactor === "number" && isFinite(portionVisionDebug.portionFactor)) {
      pfCandidate = portionVisionDebug.portionFactor;
    }
    const pfConfidence = typeof portionInsights.confidence === "number" && isFinite(portionInsights.confidence) ? portionInsights.confidence : typeof portionVisionDebug.confidence === "number" && isFinite(portionVisionDebug.confidence) ? portionVisionDebug.confidence : 0;
    if (typeof pfCandidate === "number" && isFinite(pfCandidate) && pfCandidate > 0.25 && pfCandidate < 3 && pfConfidence >= 0.6) {
    ...
    if (typeof catKey === "string") {
      if (DEFAULT_SERVINGS_BY_CATEGORY[catKey]) {
        ...
        } else if (lower.includes("burger") || lower.includes("sandwich") || lower.includes("sandwiches & burgers")) {
        ...
        } else if (lower.includes("mains") || lower.includes("dinners") || lower.includes("skillets")) {
  ...
  try {
    if (Array.isArray(fatsecretNutritionBreakdown) && fatsecretNutritionBreakdown.length > 0) {
      ...
      const rawAreas = plateComponents.map((c) => {
        const v = c && typeof c.area_ratio === "number" && c.area_ratio > 0 ? c.area_ratio : 0;
        return v;
      });
  ...
  try {
    if (selectionComponentIdsInput && Array.isArray(selectionComponentIdsInput)) {
      ...
      const validIdsSet = new Set(
        pcs.map((comp) => comp && comp.component_id).filter((id) => typeof id === "string")
      );
  ...
  const result = {
    ...
    selection_custom: selection_custom || void 0,
    debug
  };
...
function buildSelectionAnalysisResult(input, selectedComponentIds) {
  ...
  const dedupeAllergenFlags = /* @__PURE__ */ __name((flags) => {
  ...
  const pickWorstFodmapFlag = /* @__PURE__ */ __name((flagsArray) => {
    let best = null;
    let bestRank = -1;
    for (const f of flagsArray) {
      if (!f || typeof f !== "object") continue;
      const lvl = typeof f.level === "string" ? f.level : null;
      const rank = lvl && FODMAP_LEVEL_RANK[lvl] != null ? FODMAP_LEVEL_RANK[lvl] : -1;
      if (rank > bestRank) {
        bestRank = rank;
        best = f;
      }
  ...
  const pickWorstLactoseFlag = /* @__PURE__ */ __name((flagsArray) => {
    let best = null;
    let bestRank = -1;
    for (const f of flagsArray) {
      if (!f || typeof f !== "object") continue;
      const lvl = typeof f.level === "string" ? f.level : null;
      const rank = lvl && LACTOSE_LEVEL_RANK[lvl] != null ? LACTOSE_LEVEL_RANK[lvl] : -1;
      if (rank > bestRank) {
        bestRank = rank;
        best = f;
      }
  ...
  if (!result.combined_allergens && Array.isArray(input.allergen_flags) && input.allergen_flags.length > 0) {
...
function normalizeFatSecretImageResult(fsRaw) {
  ...
  const foodResponse = Array.isArray(fsRaw.food_response) ? fsRaw.food_response : [];
  ...
  foodResponse.forEach((entry, idx) => {
    ...
    const category = rawCategory === "Brand" ? "fs_brand" : rawCategory === "Generic" ? "fs_generic" : "fs_food";
    ...
    const share_ratio = totalCalories > 0 ? energyKcal / totalCalories : 1 / foodResponse.length;
...
__name(normalizeFatSecretImageResult, "normalizeFatSecretImageResult");
...
// .wrangler/tmp/bundle-Vi314h/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-Vi314h/middleware-loader.entry.ts
